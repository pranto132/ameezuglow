...
export const openMultipleInvoicesInNewTab = async (orderIds: string[]) => {
  try {
    // Fetch orders by IDs
    ...
    const invoicesHTML = generateMultipleInvoicesHTML(orders, allOrderItems || [], siteSettings);
    const newWindow = window.open("", "_blank");
    if (newWindow) {
      newWindow.document.write(invoicesHTML);
      newWindow.document.close();
    }
  } catch (error) {
    console.error("Error opening invoices:", error);
    alert("Failed to open invoices");
  }
};
...
const generateMultipleInvoicesHTML = (
  orders: Order[],
  ...
  const invoicesContent = orders.map((order) => {
    ...
    const itemsHTML = orderItems
      .map(
        ...
        <tr>
          ...
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; text-align: right;">৳${Number(item.price).toLocaleString()}</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; text-align: center;">${item.quantity}</td>
          <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; text-align: right; font-weight: 500;">৳${(Number(item.price) * item.quantity).toLocaleString()}</td>
        </tr>
      `
      )
      .join("");
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Invoices - ${orders.length} Orders</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  <div class="invoice-count">${orders.length} Invoices</div>
  <button class="print-btn" onclick="window.print()">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <polyline points="6 9 6 2 18 2 18 9"></polyline>
      <path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path>
